...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlanToFlag(userActionInput)) {
        ...
        throw new AppException("잘못된 번호 선택하였습니다.");
    }
    ...
    private static boolean doesUserChooseToPlanToFlag(String userActionInput) {
    ...
    private static void checkIfGameIsOver() {
        boolean isAllCheked = isAllCellChecked();
        if (isAllCheked) {
    ...
    private static int convertRowFrom(char cellInputRow) {
        final int rowIdex = Character.getNumericValue(cellInputRow) - 1;
        if(rowIdex >= BOARD_ROW_SIZE) {
    ...
    private static int convertColFrom(char cellInputCol) {
        switch (cellInputCol) {
            ...
            case 'j':
                return 9;
            default:
                throw new AppException("잘목된 입력입니다.");
        }
    }
